...
const addAllProducts = async (req, res) => {
  try {
    ...
    res.status(200).send({
      status: false,
      message: "Thêm sản phẩm thành côngcông!",
    });
  } catch (err) {
    res.status(500).send({
      status: false,
...
const getAllProducts = async (req, res) => {
  let { title, category, price, minPrice, maxPrice, page, limit } = req.query;
  console.log({ title, category, price, minPrice, maxPrice, page, limit });
  if (!page) {
    page = 1;
  }
  if (!limit) {
...
const deleteProduct = async (req, res) => {
  try {
    ...
    // Sử dụng await để đợi kết quả từ Mongoose
