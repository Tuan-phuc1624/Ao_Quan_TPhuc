...
const getAllOrders = async (req, res) => {
  ...
  // Kiểm tra nếu customerName là số thì thêm điều kiện cho invoice
  if (!isNaN(Number(customerName))) {
  ...
  try {
    ...
    const orders = await Order.find(queryObject)
      .select(
        "_id invoice paymentMethod subTotal total user_info discount paymentStatus confirmed shippingCost status createdAt updatedAt"
      )
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limits);
...
const getDashboardAmount = async (req, res) => {
  ...
  const getLastMonthDate = () => {
    const date = new Date();
    const firstDayLastMonth = new Date(date.getFullYear(), date.getMonth() - 1, 1); // Ngày đầu tiên của tháng trước
    const lastDayLastMonth = new Date(date.getFullYear(), date.getMonth(), 0); // Ngày cuối cùng của tháng trước
    return { firstDayLastMonth, lastDayLastMonth };
  };
  try {
    const { firstDayLastMonth, lastDayLastMonth } = getLastMonthDate();
    // total order amount
    ...
    const lastMonthOrderAmount = await Order.aggregate([
      {
        $match: {
          status: { $in: ["Delivered", "Processing"] }, // Lọc theo trạng thái Delivered và Processing
          updatedAt: {
            $gte: firstDayLastMonth, // Từ ngày đầu tháng trước
            $lte: lastDayLastMonth,  // Đến ngày cuối tháng trước
          },
        },
      },
      {
        $group: {
          _id: null,
          total: { $sum: "$total" }, // Tính tổng số tiền của các đơn hàng
        },
      },
    ]);
    ...
    res.send({
      totalAmount:
        totalAmount.length === 0
          ? 0
          : parseFloat(totalAmount[0].tAmount),
